...
class GraspNetGenerator(Node):
    ...
    def handle_grasp_request(self, request, response):
        ...
        #获取rgb和深度图
        self.rgb_image   = cv2.imread(cmd_info.get('image_rgb_url'))
        self.depth_image = cv2.imread(cmd_info.get('image_depth_url'), cv2.IMREAD_UNCHANGED)
        if self.rgb_image is None or self.depth_image is None:
            response.success = False
            response.error_message = "cannot open image."
            print(cmd_info.get('image_rgb_url'))
            print(cmd_info.get('image_depth_url'))
            return response
        
        match cmd_info.get('cmd_type'):
            case 'pickup':
                response = self.get_pickup_pose(cmd_info, response)
                return response
            case 'release':
                response = self.get_release_pose(cmd_info, response)
                return response
            case _:
                response
        
    def get_pickup_pose(self, cmd_info, response):
        # 绘制掩码框
        w,h = self.rgb_image.shape[:2]
        size = [h,w]
        ...
        # 图片转化为numpy数组
        self.rgb_image   = np.array(self.rgb_image) / 255.0
    ...
    #点云滤波
    def pcd_ground_seg_open3d(self, scan, config):
    ...
    def process_pickup_data(self):
        ...
        # 使用 Open3D 进行平面分割
        ground, rest = self.pcd_ground_seg_open3d(pcd, config)
    ...
    def get_release_pose(self, cmd_info, response):
        # 获取放物位置坐标
        # 放置点
        releasePoint = cmd_info.get('release_point')
        # 旋转安装后的相机内参
        matrix = [0.0, 606.0, 246.0,
                  -606.0, 0.0, 321.0,
                  0.0, 0.0, 1.0]
        ...
        # 转化为点云
        camera = Camerainfo(width, height, self.intrinsics, factor_depth)
        cloud = create_point_cloud_from_depth_image(self.depth_image, camera, organized=True)
        # 相机下的坐标
        releasePoint = cloud[releasePoint[1], releasePoint[0]]
        # 转化为机器人基坐标下的坐标
        grasp_target = np.array([
                    [1.000, 0.000, 0.000, releasePoint[0]],
                    [0.000, 1.000, 0.000, releasePoint[1]],
                    [0.000, 0.000, 1.000, releasePoint[2]],
                    [0.000, 0.000, 0.000, 1.000]
                    ])
        pose = self.process_model_output(grasp_target)
        if pose is None:
            response.success = False
            response.error_message = "Failed to generate grasp poses."
            response.pose = ""
    ...
    def process_model_output(self, output):
        ...
        poseout.position.x = pose_out_matrix[0, 3]
