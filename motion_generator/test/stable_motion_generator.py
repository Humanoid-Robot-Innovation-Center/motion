...
class StableMotionGenerator(MotionGenerator):
    def __init__(self):
        ...
        trim_steps = None
        max_attempts = 4
        interpolation_dt = 0.08333 # 采用12hz的频率控制
    ...
    def generate_plan_callback(self, request, response):
        ...
        #     return response
            ...
            # 测试采用不同方法生成目标位姿
            goal_pose = Pose.from_list([target_position.x, target_position.y, target_position.z + 0.0, target_orientation.w, target_orientation.x, target_orientation.y, target_orientation.z])
    ...
    def joint_states_callback(self, future):
        ...
        # 由于curobo的配置文件中去除了如头部电机这类对任务执行无关的关节，同时伸缩臂多出了两个关节joint_arm_l1，joint_arm_l2，这里需要对获取到的关节状态进行处理
