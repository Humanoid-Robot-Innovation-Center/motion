...
from .helpers import fit_plane as fp


class ArucoMarker:
    # 根据传入的aruco_id和yaml文件中对应id的信息生成一个二维码对象
    def __init__(self, aruco_id, marker_info, show_debug_images=False):
        ...
        # todo:后期urdf修改需要修改这个地方相机坐标的id
        self.frame_id = 'camera_color_optical_frame'
        # 从配置文件传入ros的参数获取marker相关信息（长度，id）
        ...
        self.ready = False  # 该marker对象接口可以被调用的标志

    def update_marker_point_cloud(self, aruco_depth_estimate):
        # 从 ArUco marker 所在区域的深度图像中提取三维点云数据
    ...
    def update(self, corners, timestamp, frame_number, camera_info, depth_image=None):
        ...
        rvecs = np.zeros((len(self.corners), 1, 3), dtype=np.float64)  # 该marker的旋转向量
        tvecs = np.zeros((len(self.corners), 1, 3), dtype=np.float64)  # 该marker的平移向量
        points_3D = np.array([
        ...
        aruco_depth_estimate = self.aruco_position[2]  # 从rgb图获取的深度估计
        ...
        if (self.depth_image is not None) and (aruco_depth_estimate > self.min_z_to_use_depth_image) and (
        not self.use_rgb_only):
            only_use_rgb = False
            ...
            if num_points < min_number_of_points_for_plane_fitting:
                print(
                    'WARNING: There are too few points from the depth image for plane fitting, so only using the RGB ArUco estimate. number of points =',
                    num_points)
                only_use_rgb = True
        else:
            only_use_rgb = True
        ...
        if not only_use_rgb:
            ...
            if (x_axis is not None) and (y_axis is None):
                ...
                if (x_axis is None) and (y_axis is None):
                    print(
                        'WARNING: The detected ArUco corners did not project to reasonable 3D points on the fit plane.')
...
class ArucoMarkerCollection:
    ...
    def broadcast_tf(self, tf_broadcaster):
        ...
        for key in self.collection:
            ...
            # print(marker.aruco_id)
            # print(marker.marker_position[0]) # x
            # print(marker.marker_position[1]) # y
            # print(marker.marker_position[2])  # z
            marker.broadcast_tf(tf_broadcaster)

    def update(self, rgb_image, camera_info, depth_image=None, timestamp=None):
        # 从rgb及深度图中检测二维码的位姿及id,保存到字典中
        self.frame_number += 1
        self.timestamp = timestamp
        self.rgb_image = rgb_image
        self.camera_info = camera_info  # 字典
        self.depth_image = depth_image
        ...
        if self.aruco_ids is not None:
            for corners, aruco_id in zip(self.aruco_corners, self.aruco_ids):
                ...
                self.collection[aruco_id].update(corners, self.timestamp, self.frame_number, self.camera_info,
                                                 self.depth_image)

    def get_ros_marker_array(self):
        # 获取rviz可视化marker坐标的接口
        # 输出将当前帧的所有二维码rviz可视化消息
...
class DetectArucoNode(Node):
    # 输入深度图，rgb图，广播marker到相机坐标的tf以及相应的可视化消息
    def __init__(self):
        ...
        self.rgb_topic_name = '/head_camera/color/image_raw' #'/camera/infra1/image_rect_raw'
        self.rgb_image_subscriber = message_filters.Subscriber(self, Image, self.rgb_topic_name)

        self.depth_topic_name = '/head_camera/aligned_depth_to_color/image_raw'
        self.depth_image_subscriber = message_filters.Subscriber(self, Image, self.depth_topic_name)

        self.camera_info_subscriber = message_filters.Subscriber(self, CameraInfo, 'head_camera/aligned_depth_to_color/camera_info')

        self.synchronizer = message_filters.ApproximateTimeSynchronizer([self.rgb_image_subscriber, self.depth_image_subscriber, self.camera_info_subscriber], 10, 1.0, allow_headerless=True)
    ...
    def image_callback(self, ros_rgb_image, ros_depth_image, rgb_camera_info):
        # 将时间相近的rgb图，深度图，相机内参作为输入进行二维码检测和处理
        try:
            self.rgb_image = self.cv_bridge.imgmsg_to_cv2(ros_rgb_image, 'bgr8')
            # 根据相机竖向安装方式，旋转rgb图像 90 度
            self.rgb_image = cv2.rotate(self.rgb_image, cv2.ROTATE_90_COUNTERCLOCKWISE)
            self.rgb_image_timestamp = ros_rgb_image.header.stamp
            self.depth_image = self.cv_bridge.imgmsg_to_cv2(ros_depth_image)
            # 根据相机竖向安装方式，旋转depth图像 90 度
            self.depth_image = cv2.rotate(self.depth_image, cv2.ROTATE_90_COUNTERCLOCKWISE)
            self.depth_image_timestamp = ros_depth_image.header.stamp
        except CvBridgeError as error:
            print(error)

        # 获取相机的内参并交换 fx, fy, cx, cy
        self.camera_info = rgb_camera_info
        K = self.camera_info.k

        # 交换 fx 和 fy
        K[0] = rgb_camera_info.k[4]  # fx
        K[4] = rgb_camera_info.k[0]  # fy

        # 交换 cx 和 cy
        K[2] = rgb_camera_info.k[5]  # cx
        K[5] = rgb_camera_info.k[2]  # cy
        ...
        # 传入aruco_marker_collection数据结构
        self.aruco_marker_collection.update(self.rgb_image, self.camera_info, self.depth_image,
                                            self.rgb_image_timestamp)
    ...
    def add_to_point_cloud(self, x_mat, y_mat, z_mat, mask):
        points = [[x, y, z] for x, y, z, m in zip(x_mat.flatten(), y_mat.flatten(), z_mat.flatten(), mask.flatten()) if
                  m > 0]
