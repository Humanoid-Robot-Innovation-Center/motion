...
class StableMotionGenerator(MotionGenerator):
    def __init__(self):
        ...
        trim_steps = None
        max_attempts = 8
        interpolation_dt = 1/5
        #获取机器人配置
        robot_path = os.path.join(package_share_directory, 'data', 'konka.yml')
        self.robot_cfg = load_yaml(robot_path)["robot_cfg"]
        robot_asset_path = os.path.join(package_share_directory, 'data', 'konka_description')
        self.robot_cfg["kinematics"]["external_asset_path"] = robot_asset_path
    ...
    def generate_plan_callback(self, request, response):
        ...
        #     return response
            ...
            if succ:
                ...
                # 记录末端轨迹
                plan_in_world = self.cmd_plan
                # plan_in_world = np.array(plan_in_world.ee_pos_seq.cpu(), dtype=np.float32)
                plan_in_world.position[0] += self.base_z_inWorld
                end_eff_seq = self.motion_gen.compute_kinematics(plan_in_world)
                end_eff_seq = np.array(end_eff_seq.ee_pos_seq.cpu(), dtype=np.float32)
                response.ee_pos_x = end_eff_seq[:,0].tolist()
                response.ee_pos_y = end_eff_seq[:,1].tolist()
                response.ee_pos_z = end_eff_seq[:,2].tolist()
                return response
            else:
    ...
    def update_joint_callback(self, future):
        ...
        for joint_name in joint_names:
            ...
            # 因此，调用规划器时，应当更新感知数据，否则会出错
                ...
                filtered_joint_velocities.append(joint_velocities[index])
                self.base_z_inWorld = joint_positions[index]
            else:
